...
REF_DIR=${REF:-/opt/jenkins-dist/ref/plugins}
...
bundledPlugins() {
    local JENKINS_WAR=/opt/jenkins-dist/jenkins.war
    if [ -f $JENKINS_WAR ]
    then
...
jenkinsMajorMinorVersion() {
    local JENKINS_WAR
    JENKINS_WAR=/opt/jenkins-dist/jenkins.war
    if [[ -f "$JENKINS_WAR" ]]; then
        local version major minor
        version="$(java -jar /opt/jenkins-dist/jenkins.war --version)"
